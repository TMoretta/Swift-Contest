...
class ParticipantWorkSubmitPage extends StatefulWidget {
  final Map<String,dynamic> extendedContestJson;
  const ParticipantWorkSubmitPage({required this.extendedContestJson, super.key});
...
class _ParticipantWorkSubmitPageState extends State<ParticipantWorkSubmitPage> {
  ...
  Widget build(BuildContext context) {
    final extendedContest =ExtendedContest.fromJson(widget.extendedContestJson);
    return BlocProvider(
      create: (context) => getIt<ParticipantWorkSubmitPageBloc>(),
                ...
                if (formKeys[currentStep].currentState?.validate() ?? false) {
                  if (isLastStep) {
                    ...
                    context.read<ParticipantWorkSubmitPageBloc>().add(ParticipantWorkSubmitPageSubmitWork(
                      contestId: extendedContest.contest.id,
                      ...
                      ElevatedButton(
                        ...
                        child: isLastStep
                          ...
                          listener: (context, state) {
                            if (state is ParticipantWorkSubmitPageSuccess) {
                              showSnackBar(
                          ...
                          builder: (context, state) {
                            if (state is ParticipantWorkSubmitPageLoading) {
                              return Loader();
                            }
                            return Text('Submit');
                          },
