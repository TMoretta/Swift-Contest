...
class _OrganizerJuryDetailsPageState extends State<OrganizerJuryDetailsPage> {
  ...
  bool formHasBeenUpdated = false;
  bool juryHasBeenDeleted = false;
    ...
    return BlocConsumer<OrganizerJuryDetailsPageBloc, OrganizerJuryDetailsPageState>(
      listener: (context, state) {
        ...
        if (state.status.isSuccess && state.sourceEvent is OrganizerJuryDetailsPageDeleteJury) {
          context.router.pop(); // pop from the dialog
          showSnackBar(context: context, text: 'Jury deleted successfully');
          juryHasBeenDeleted = true;
        }
      },
      builder: (context, state) {
        return PopScope(
          ...
          onPopInvokedWithResult: (didPop, result) {
            if(didPop) return;
            // return true if form has been updated or jury has been deleted to refresh the
            // contest details page
            context.router.pop(formHasBeenUpdated || juryHasBeenDeleted);
          },
          child: Scaffold(
            appBar: CustomAppBar(
                              ...
                              children: [
                                ...
                                Scaffold(
                                  ...
                                  floatingActionButton: (state.isInitialized)
                                      ? FloatingActionButton.extended(
                                          onPressed: () async {
                                            final bool? formUpdated = await context.router.push(
          ...
          listener: (context, state) {
            if (state.status.isSuccess &&
              ...
              context
                  .read<OrganizerJuryDetailsPageBloc>()
                  .add(OrganizerJuryDetailsPageFetch(juryId: juryId));
              context.router.pop(); // pop the dialog
            }
          },
          builder: (context, state) {
            return AlertDialog(
