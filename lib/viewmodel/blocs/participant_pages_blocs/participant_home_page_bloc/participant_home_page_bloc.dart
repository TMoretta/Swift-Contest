...
class ParticipantHomePageBloc extends Bloc<ParticipantHomePageEvent, ParticipantHomePageState> {
  ...
  ParticipantHomePageBloc({
    ...
    required JurationRepository jurationRepository,
        ...
        super(ParticipantHomePageInitial()) {
  ...
  Future<void> _getJoinedContestsExtended(
    ...
    emit(ParticipantHomePageLoading());
    ...
    resOwnParticipations.fold(
      (failure) => emit(ParticipantHomePageFailure(message: failure.message)),
    ...
    emit(ParticipantHomePageSuccess(
      contests: contests,
    ...
    // emit(ParticipantHomePageLoading());
    // final res = await _contestRepository.getExtendedContestsByParticipantId(
    //     participantId: event.participantId);
    // res.fold(
    //   (failure) => emit(ParticipantHomePageFailure(message: failure.message)),
    //   (success) => emit(ParticipantHomePageSuccess(extendedContests: success)),
    // );
  }
  ...
  Future<void> _joinContest(
    ...
    res.fold(
      (failure) => emit(ParticipantHomePageFailure(message: failure.message)),
      (success) => emit(ParticipantHomePageSuccess(participation: success)),
    );
  }
}
