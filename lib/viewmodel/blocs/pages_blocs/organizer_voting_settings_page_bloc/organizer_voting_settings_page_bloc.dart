...
class OrganizerVotingSettingsPageBloc
  ...
  FutureOr<void> _createVotingSessionAndBeginProcedure(
    ...
    for (var vff in oldVotingFormFields) {
      final eitherVotingFormField = await _votingFormFieldRepository.createVotingFormField(
        votingFormField: VotingFormField(
          ...
          fieldType: vff.fieldType,
          isOptional: vff.isOptional,
          name: vff.name,
    ...
    final eitherVotingSession = await _votingSessionRepository.createVotingSession(
      ...
      isSimpleJurorVotingAllowed: event.isSimpleJurorAllowed,
      votingFormId: votingForm.id,
    ...
    for (var i = 0; i < event.votingParticipants.length; i++) {
      ...
      final eitherVotingSessionParticipant =
          await _votingSessionParticipantRepository.createVotingSessionParticipant(
    ...
    for (var juror in event.votingJurors) {
      final eitherVotingSessionJuror = await _votingSessionJurorRepository.createVotingSessionJuror(
    ...
    for (var votingJuror in event.votingJurors) {
      for (var votingParticipant in event.votingParticipants) {
        ...
        if (event.votingExclusions
            .contains(ParticipantAndJuror(participant: votingParticipant, juror: votingJuror))) {
          isExcluded = true;
        }
        ...
        final eitherVotingSessionParticipant = await _votingSessionParticipantRepository
        ...
        final eitherVoting = await _votingRepository.createVoting(
          voting: Voting(
            id: genUuid(),
            createdAt: DateTime.now(),
            votingSessionId: votingSession.id,
    ...
    final eitherToken = await _utilsRepository.genUniqueToken(tableName: 'voting_session_tokens', columnName: 'token', length: 8);
    eitherToken.fold(
        (failure) => emit(state.copyWith(status: BlocStatus.failure,message: failure.message)),
    ...
    final eitherVotingSessionToken = await _votingSessionTokenRepository.createVotingSessionToken(votingSessionToken: VotingSessionToken(id: genUuid(), createdAt: DateTime.now(), votingSessionId: votingSession.id, token: token,));
