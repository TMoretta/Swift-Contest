...
serve(async (req: Request) => {
  ...
  try {
    const { p_juror_invitation } = await req.json();
    const { email, jury_id } = p_juror_invitation;
...
//         p_table: "participant_invitations",
//         p_column: "token",
//         p_length: 14,
//       });
//
...
//     }
    ...
    // 2) Inserisci l’invitation e prendi subito il token
    const { data: insertedInvitations, error: insertError } = await supabase
      ...
      .insert(
        ...
        { returning: "representation" }   // <- qui chiedi di restituire la riga completa
      );
    ...
    const subject = "You have been invited to vote in a contest";
    const html = `
      <h2>Welcome!</h2>
      <p>You’ve been invited as a <strong>juror</strong>.</p>
