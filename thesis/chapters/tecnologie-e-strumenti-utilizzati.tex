...
\begin{itemize}
	...
	È stata scelta per la sua integrazione nativa con i framework frontend moderni, la sua pipeline di CI/CD automatizzata e la sua capacità di gestire in modo ottimale le Single Page Applications (SPA) come Flutter Web.
	
	\item \textbf{Resend}: È il servizio transazionale di invio \textbf{email} utilizzato dal sistema.
	È stato integrato tramite la sua API REST, invocata da una Edge Function, per gestire l'invio di tutte le email automatiche, come gli inviti per partecipanti e giurati.
	...
	Questo servizio è stato integrato per fornire la funzionalità di ricerca e autocompletamento degli indirizzi nella creazione dei contest e nella configurazione delle sessioni di voto per la geo-localizzazione.
\end{itemize}
