...
\begin{itemize}
    ...
    \item \textbf{Sicurezza ``\emph{by design}''}: adottare un'architettura di sicurezza basata sul \textbf{Principio del Minimo Privilegio (Principle of Least Privilege)}, dove l'accesso al database è negato per default e concesso selettivamente solo tramite un'API controllata.
...
\begin{itemize}
    \item \textbf{Proposta di una nuova categoria di prodotto}: Data l'assenza di soluzioni verticali, il progetto stesso si configura come un caso di studio per una nuova categoria di software dedicato alla gestione dei \emph{contest}, colmando un vuoto di mercato.
    \item \textbf{Dimostrazione di un'architettura sicura su Backend-as-a-Service (BaaS)}: Il sistema mostra come implementare un'architettura basata sul Principio del Minimo Privilegio utilizzando una piattaforma Backend-as-a-Service (BaaS).
...
\begin{itemize}
    ...
    \item \textbf{Capitolo 5 – Distribuzione e prodotto finale}: presenta il processo di deploy, l’interfaccia utente e i flussi di utilizzo reali.
